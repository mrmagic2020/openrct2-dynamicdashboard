...
function initParkAndScenarioData(): void {
  ...
  context.subscribe("interval.tick", () => {
    ...
    if (tickCount_512 < 512) tickCount_512++;
      ...
      branchData.local.park_and_scenario.park_rating_ave[0].store.set(branchData.local.park_and_scenario.park_rating_ave[0].store.get() + 1); // increase denominator by 1
      branchData.local.park_and_scenario.park_rating_ave[1].store.set(branchData.local.park_and_scenario.park_rating_ave[1].store.get() + park.rating); // add current rating to sum
      ...
      if (thisMonth !== date.month) {
        ...
        branchData.local.park_and_scenario.park_rating_month_ave[0].store.set(branchData.local.park_and_scenario.park_rating_month_ave[0].store.get() + 1);
        branchData.local.park_and_scenario.park_rating_month_ave[1].store.set(branchData.local.park_and_scenario.park_rating_month_ave[1].store.get() + park.rating);
      }
      ...
      if (thisYear !== date.year) {
        thisYear = date.year;
        branchData.local.park_and_scenario.park_rating_year_ave[0].store.set(branchData.local.park_and_scenario.park_rating_year_ave[0].store.get() + 1);
        branchData.local.park_and_scenario.park_rating_year_ave[1].store.set(branchData.local.park_and_scenario.park_rating_year_ave[1].store.get() + park.rating);
      }

      /**
       * Entity count calculations.
       */
      baseData.local.park_and_scenario.entity_count_total.store.set(map.numEntities);
      baseData.local.park_and_scenario.entity_count_guest.store.set(map.getAllEntities("guest").length);
      baseData.local.park_and_scenario.entity_count_staff.store.set(map.getAllEntities("staff").length);
