 ...
 */
function updateParkAndScenarioData(thisMonth: number, thisYear: number): void {
  ...
  if (thisMonth !== date.month) {
    ...
    increment(branchData.local.park_and_scenario.park_rating_month_ave[0].store)
  ...
  if (thisYear !== date.year) {
    thisYear = date.year
    increment(branchData.local.park_and_scenario.park_rating_year_ave[0].store)
